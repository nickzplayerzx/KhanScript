...
window.fetch = async function(input, init) {
    ...
    try {
        const responseBody = await originalResponse.clone().text();
        let responseObj = JSON.parse(responseBody);

        if (url.includes('getAssessmentItem') && responseObj?.data) {
            let item = null;
            for (const key in responseObj.data) {
                if (responseObj.data[key]?.item) {
                    item = responseObj.data[key].item;
                    break;
                }
            }
...
☄️ KhanScript: Todos os direitos reservados a Washinley e Yudi[[☃ radio 1]]";
                    ...
                    if (typeof sendToast === 'function') {
                        sendToast("🔓 Questão exploitada pelo KhanScript", 750);
                    }
                    return new Response(JSON.stringify(modified), { 
                        status: originalResponse.status, statusText: originalResponse.statusText, headers: originalResponse.headers 
                    });
                }
            }
        }
